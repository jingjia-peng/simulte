...
void LteRlcUmRealisticD2D::initialize(int stage)
{
    if (stage == 0)
    {
        ...
        {
            nodeType_ = UE;
//            IPHACK: add the IP-based mode4 Mac interface "LteMacUeMode4"
            if (macType.compare("LteMacUeRealisticD2D") != 0 && macType.compare("LteMacVUeMode4D2D") != 0
                    && macType.compare("LteMacVUeMode4") != 0 && macType.compare("LteMacVUeMode4NonSPS") != 0)
